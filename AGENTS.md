...
**Case content:** every case must be a real historical/legal case with a Wikipedia article as its primary source (`cases.source_url`). Case briefs are original prose paraphrasing the article, never copied verbatim — see DECISIONS.md and TIN-469. Every case also needs `tldr` (one hook sentence) and `key_facts` (bulleted, scannable) — shown before the full narrative brief, which is collapsed by default. **No real photos** — licensing risk, same reasoning as the Wikipedia policy; use generated visual treatment (gradient header, icons) instead, see DECISIONS.md. All of `brief`/`evidence`/`counter_arguments`/`tldr`/`key_facts` must never state or imply the real outcome — that's `real_verdict`/`historical_context` only, reveal-only fields. **The `source_url` link itself is reveal-only too** — it's a direct link to the real outcome, so it must never be clickable before resolution (`CaseBrief`, shown pre-vote, has no source link; `Reveal`'s Layer 3 is the only place it appears).

**Theme:** dark only, no light-mode variant, no `dark:` Tailwind classes anywhere. Do not reintroduce `@media (prefers-color-scheme: dark)` in globals.css — caused a real invisible-text bug (component text hardcoded for a dark backdrop, page shell following the system's light preference).
